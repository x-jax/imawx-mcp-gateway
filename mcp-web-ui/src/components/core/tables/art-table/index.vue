    ...
    <ElTable ref="elTableRef" v-loading="!!loading" v-bind="mergedTableProps">
      <template v-for="col in columns" :key="col.prop || col.type">
        ...
        <ElTableColumn v-else v-bind="cleanColumnProps(col)">
          ...
          <!-- 兜底:用 formatter(row) 渲染自定义内容(2026-07-03 加) -->
          <!-- 之前只有 expand 类型列支持 formatter,普通列即使写了 formatter 也被 ArtTable 忽略, -->
          <!-- 导致 cells 为空。ColumnOption.formatter 是自定义 API:返 VNode(组件树)/ string / number, -->
          <!-- 都不走 el-table 原生 formatter(返 string)。用 FormatterRender wrapper 区分两种返回:VNode 透传, -->
          <!-- primitive 转 span,避免 <component :is> 把字符串当 HTML tag 报 InvalidCharacterError。 -->
          <template v-if="col.formatter && col.prop" #default="{ row }">
            <FormatterRender :formatter="col.formatter" :row="row" />
          </template>
        </ElTableColumn>
      </template>
...
<script setup lang="ts">
  import { ref, computed, nextTick, watchEffect, getCurrentInstance, useAttrs, h, defineComponent, isVNode } from 'vue'
  ...
  /**
   * 通用 formatter 渲染 wrapper(2026-07-03 加)—— ColumnOption.formatter 返 VNode / string / number,
   * 用一个 inline 组件统一处理:
   * - VNode → 透传(返回原 VNode,Vue 直接渲染子树)
   * - primitive → 包 <span> 显示,避免 <component :is="字符串"> 把内容当 HTML tag
   *
   * <p>放在 ArtTable 内联定义(不在全局注册)是因为只有表格列用得到;且每个 cell 调用
   * formatter 时 setup 才执行,不会引起额外的全表性能开销。
   */
  const FormatterRender = defineComponent({
    name: 'FormatterRender',
    props: {
    ...
    setup(props) {
      return () => {
        const result = props.formatter(props.row)
  ...
  const cleanColumnProps = (col: ColumnOption) => {
    ...
    // 删除自定义 formatter(2026-07-03 加)—— 这是 ColumnOption 的 VNode 渲染函数,
    // 不走 el-table 原生 formatter(返 string),避免冲突导致 cells 为空 / [object Object]。
