...
async function handleRouteGuard(
  ...
  // 1. 检查登录状态；刷新后本地状态丢失时，先通过后端 session 恢复账号信息。
  ...
  // 4. 首登或密码重置后必须先改密码
  if (handlePasswordChangeRequired(to, userStore, next)) {
    return
  }
...
function handlePasswordChangeRequired(
  ...
  if (!userStore.isLogin || to.path === PASSWORD_CHANGE_PATH) {
    return false
  }
  if ((userStore.getUserInfo as any)?.mustChangePassword) {
    next({ path: PASSWORD_CHANGE_PATH, replace: true })
    return true
  }
  return false
}
...
async function handleDynamicRoutes(
  ...
  try {
    ...
    const userStore = useUserStore()
    if ((userStore.getUserInfo as any)?.mustChangePassword && to.path !== PASSWORD_CHANGE_PATH) {
      routeInitInProgress = false
      next({ path: PASSWORD_CHANGE_PATH, replace: true })
      return
    }
    ...
    if (to.matched.some((record) => record.name === 'Exception404')) {
      routeInitInProgress = false
      next({
        path: to.path,
        query: to.query,
