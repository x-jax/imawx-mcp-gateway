...
export function handleError(error: AxiosError<ErrorResponse>): never {
  ...
  // 处理 HTTP 状态码错误 — 优先用后端业务 message(对齐 ResponseEnvelope.message 字段),
  // 没给时才 fallback 到 i18n mapping(覆盖 transport-level 错误如 502/504 这种后端
  // 没业务 R 包的场景,或者 axios 自身抛的 timeout / network error)。
  const message =
